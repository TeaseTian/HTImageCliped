...
/**
 * GITHUB           : https://github.com/CoderJackyHuang/HTImageCliped
 * Chinese Document : http://www.henishuo.com/HTimagecliped-image-cornerradius/
 ...
 */
typedef void(^HTClipedCallback)(UIImage *clipedImage);
 ...
 if (model.clipedImage) {
   ...
   [self.imageView sd_setImageWithURL:[NSURL URLWithString:model.url] placeholderImage:image options:SDWebImageRetryFailed completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
     dispatch_async(dispatch_get_global_queue(0, 0), ^{
       ...
       // 将剪裁后的图片记录下来，下次直接使用
       model.clipedImage = [image HT_clipToSize:weakSelf.imageView.bounds.size
       cornerRadius:12
 ...
 */
@interface UIView (HTImageCliped)

#pragma mark - 边框相关属属性，仅对生成圆形图片和矩形图片有效
/**
 *	默认为0.0，当小于0时，不会添加边框，仅对生成圆形图片和矩形图片有效
 */
@property (nonatomic, assign) CGFloat HT_borderWidth;
/**
 *	当小于0时，不会添加边框。默认为0.仅对生成圆形图片和矩形图片有效
 */
@property (nonatomic, assign) CGFloat HT_pathWidth;
/**
 *	边框线的颜色，默认为[UIColor lightGrayColor]，仅对生成圆形图片和矩形图片有效
 */
@property (nonatomic, strong) UIColor *HT_borderColor;
/**
 *	Path颜色，默认为白色。仅对生成圆形图片和矩形图片有效
 */
@property (nonatomic, strong) UIColor *HT_pathColor;
 ...
 *  默认为NO
 */
@property (nonatomic, assign) BOOL HT_shouldRefreshCache;
 ...
 *
 *  @Example             
 *  [cornerView3 HT_setImage:@"bimg8.jpg" cornerRadius:10 rectCorner:UIRectCornerTopLeft |UIRectCornerBottomRight isEqualScale:NO onCliped:^(UIImage *clipedImage) {
 ...
 *
 * @Example   
 * 添加一个圆角：[view1 HT_addCorner:UIRectCornerTopLeft cornerRadius:10];
 */
 ...
 */
- (void)HT_addCorner:(UIRectCorner)corner
